...
**Never build a find string out of a table row.** The export renders a native table as `| a | b |`, but the live document has cells and contains no `|` there — so a piped row pre-flights as unique and then pushes `occurrencesChanged: 0`, silently. Measured live. Target the text of a *single cell*, which is real text. `checkfind` now refuses this shape (`TRIP: find-spans-table-cells`), but the check is narrow by necessity: a pipe can be genuine body text, so it only fires when the matched line is itself a table row.

**The export flattens line breaks *inside* a cell, and nothing downstream can see it.** The export is faithful about table *structure* — real pipe rows — but not about line structure within a cell: a cell holding a ten-item list comes back as one space-joined line. A local mirror written from that export matches it, so the diff reports **no hunk at all**, and the two sides look identical while they are not. Replace the whole cell from that single-line source and the list collapses — and because the post-push re-export flattens the damage identically, the Phase 6 verification cannot see it either. **This is the only corruption in this system that cannot be caught after the fact.** So it is a **judgement call you have to make, not a gate that will stop you**: before replacing the whole of a table cell, read that cell with `docs_get_table` and confirm it is genuinely single-line. **Editing text within one line of a cell is always safe** — only the whole-cell replace destroys structure, and targeting a substring inside the cell avoids the question entirely.

**Why there is no guard here.** One was built and reverted (2026-07-28). A single-line cell and a flattened multi-line one are indistinguishable in the export, so the check had to fire on *every* whole-cell find. Measured across two real documents: the median cell is **12 characters**, and one of them had **1 cell over 120 chars out of 128** — about 1% precision. It also converted pushes already verified against production into refusals. A gate that stops you 127 times out of 128 gets waved through on the 128th, which is worse than no gate. Narrowing it by cell length or word count would be inferring cell shape from text, which this engine does not do. If a signal appears that does not require guessing — a cell read that preserves line breaks, or line-addressed writes — the guard becomes worth rebuilding.
...
| A cell read that preserves intra-cell line breaks, **or** a write that addresses a cell's lines | The intra-cell-flattening warning — and this is the signal that would make the reverted guard worth rebuilding, since it removes the need to infer cell shape from text |
